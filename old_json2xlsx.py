...
def main():
	...
	try:
		...
		print(f"Nenhum argumento informado.\nSintaxe: {sys.argv[0]} [nome_report] [tag]")
		exit(1)

	filel=glob.glob(f"./json/{runTag}/*.json")
	...
	for jfile in filel:
		...
		if len(workSheetName) > 31: workSheetName = "..."+workSheetName[-28:]
		...
		except xw.exceptions.DuplicateWorksheetName:
			ws = wb.add_worksheet(workSheetName[:28]+"(1)")
		...
		ws.merge_range('D1:P1', f"CookieScanner - Relatório extraído em {dtCriacaoJSON}", dummyFormat)
		...
		ws.insert_image(f"A{xrow+2}", f"./img/{runTag}/"+ntpath.basename(jfile)[0:-5]+".png")
